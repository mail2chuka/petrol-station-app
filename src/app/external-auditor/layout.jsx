...
const menuItems = [
  ...
  { label: 'Daily Report', href: '/external-auditor/daily' },
  { label: 'Monthly Report', href: '/external-auditor/monthly' },
];
...
export default function ExternalAuditorLayout({ children }) {
  ...
  useEffect(() => {
    ...
    else if (session.user.role !== 'external_auditor') router.push('/');
  }, [session, status, router]);

  if (status === 'loading' || !session || session.user.role !== 'external_auditor') {
    return (
