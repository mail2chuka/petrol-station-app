...
export default function AuditorTransactionsPage() {
  ...
  useEffect(() => {
    ...
    fetchTx();
  }, [session, status, fetchTx]);

  if (status === 'loading') return <PageLoader />;

  const TYPE_COLORS = { topup: 'text-emerald-600', order: 'text-red-600', refund: 'text-blue-600', adjustment: 'text-amber-600' };

  const columns = [
    { header: 'Customer', render: (r) => r.customerName },
    { header: 'Type', render: (r) => <span className={`capitalize font-medium ${TYPE_COLORS[r.type] || ''}`}>{r.type}</span> },
    {
      header: 'Amount',
      render: (r) => (
    ...
    { header: 'Balance After', render: (r) => `₦${r.balanceAfter.toLocaleString()}` },
