  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/materials/orders', {
        ...
        body: JSON.stringify({ items: validItems, notes, confirmOrder: true }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      setSuccess('Order confirmed successfully!');
      setTimeout(() => router.push('/materials/customer/orders'), 1200);
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-100">
      ...
      <main className="mx-auto max-w-3xl px-4 sm:px-6 py-8">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-2xl font-bold text-slate-900">Place New Order</h1>
        </div>
