...
function SummaryBookContent() {
  ...
  const columns = [
    ...
    { header: 'Opening Stock (L)', render: (r) => `${(r.openingStock ?? 0).toFixed(2)} L` },
    { header: 'Stock In (L)', render: (r) => `${(r.stockIn ?? 0).toFixed(2)} L` },
    {
      header: 'Tolerance (L)',
      render: (r) => {
        const tolerance = (r.sales ?? 0) - ((r.openingStock ?? 0) + (r.stockIn ?? 0) - (r.closingStock ?? 0));
        return `${tolerance.toFixed(2)} L`;
      },
    },
    { header: 'Sales (L)', render: (r) => `${(r.sales ?? 0).toFixed(2)} L` },
    ...
    { header: 'Shortage Recorded (L)', render: (r) => `${(r.shortage ?? 0).toFixed(2)} L` },
    { header: 'Tank Closing Stock (L)', render: (r) => `${(r.closingStock ?? 0).toFixed(2)} L` },
  ];
