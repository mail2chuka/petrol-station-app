...
// Builds one row per shift per product for a station across a date range —
// the shared aggregation behind both the on-screen summary book (JSON API)
// and its Excel/PDF export, so the two never drift out of sync again.
// Returns { station, rows }. Rows are in build order (grouped by shift, not
// sorted for presentation) — callers sort for their own audience: the JSON
// API shows latest-first for on-screen browsing, while the export keeps
// chronological order since a printed ledger reads start-to-end.
export async function buildSummaryBookRows(stationId, from, to) {
